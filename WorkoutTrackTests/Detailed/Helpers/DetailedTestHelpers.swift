...
func anyDetail(id: UUID = UUID(), time: Date = Date()) -> (model: Detailed, local: DetailedDTO) {
    let model = Detailed(uid: id, setName: "", weight: 10, isDone: true, reps: 10, id: id.uuidString, time: time)
