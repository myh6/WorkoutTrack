...
/**Not Using For Now**/
class CollapsibleSetView: UIView {
    ...
    private lazy var firstHeightConstraint = firstView.heightAnchor.constraint(equalToConstant: (superview?.frame.height ?? 500)*0.55)
    ...
    @objc func handleShow() {
        if shouldCollapse {
            animateView(isCollapse: false,
                        heightConstraint: (superview?.frame.height ?? 500)*0.55,
                        heightForSection: 0)
        } else {
            animateView(isCollapse: true,
                        heightConstraint: 0,
                        heightForSection: (superview?.frame.height ?? 500)*0.55)
        }
    }
