...
protocol AddCustomActionCellDelegate: AnyObject {
    func addCustomActionToData()
}

class AddCustomActionCell: UITableViewCell {
    
    //MARK: - Properties
    private lazy var addButton: UIButton = {
        let button = UIButton()
        var config = UIButton.Configuration.plain()
        var text = AttributedString(" Add Custom Action")
    ...
    weak var delegate: AddCustomActionCellDelegate?
    //MARK: - Lifecycle
    ...
    @objc func handleAddCustomAction() {
        delegate?.addCustomActionToData()
    }
